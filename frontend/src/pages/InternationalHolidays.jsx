...
const InternationalHolidays = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const internationalPackages = packages.filter(pkg => 
        pkg.region === 'international' || pkg.region === 'both'
      );
      setAllPackages(internationalPackages);
      
      // Get trending packages
      if (pageConfig.trending.enabled && pageConfig.trending.packageIds.length > 0) {
        const trending = internationalPackages.filter(pkg => 
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading International Holidays...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Helmet>
        <title>{`${config.seo_title || 'International Holidays'} — Suvidha Travel`}</title>
